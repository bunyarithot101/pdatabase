...
class PokemonEvol3rdAdapter(
    private val context: Context,
    private val dataset: storageEvolvesTo3rd?,
    private val color: String,
    private val args: detailPokemonFragmentArgs?,
    private val onItemCardEvoleClick: onCardEvol3rd
) : RecyclerView.Adapter<PokemonEvol3rdAdapter.ItemViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
        ...
        if( dataset?.evolvesTo?.size!! > 2){
            val adapterLayout = LayoutInflater.from(parent.context)
                .inflate(R.layout.poke_evol_more_than_one, parent, false)
        }
    ...
    override fun getItemCount() = dataset?.evolvesTo?.size!!

    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        val items = dataset?.evolvesTo?.get(position)

        holder.name.text = items?.species?.name?.toUpperCase()
//        holder.condition.text = items?.evolutionDetails?.first()?.trigger?.name


        loadArtwork(holder, items?.species?.name.toString())
        loadColor(color, holder)

        getCondition(items?.evolutionDetails?.first()!!, holder)
        getNumber(items?.species?.name, holder)

        if(args!!.pokemon.name != items.species.name){
            holder.itemView.setOnClickListener {
                onItemCardEvoleClick.onCardEvol3rdClick(items.species.name)
            }
        }
...
//                holder.itemView.setOnClickListener {
//                    onItemCardEvoleClick.onCardEvol3rdClick(0,data.id.toInt())
//                }
    ...
    fun getTypes(PokemonNumber: Int, holder: PokemonEvol3rdAdapter.ItemViewHolder){
    ...
    fun getCondition(
        ...
        holder: PokemonEvol3rdAdapter.ItemViewHolder
    ) {
//        holder.condition.text = text
        ...
        Timber.i("getCondition %s", condition)
